...
BOARD_PX = 540   # canvas size in pixels (square)

def draw_board(target_pos=None, result_pos=None):
    """Return a PIL Image of the dartboard with optional click markers."""
    size   = BOARD_PX
    img    = Image.new("RGB", (size, size), "black")
    draw   = ImageDraw.Draw(img)
    cx, cy = size // 2, size // 2
    R      = size // 2 - 30   # outer_radius in pixels
    ...
    for i in range(20):
        ...
        for s in range(steps + 1):
            a  = a0 + (a1 - a0) * s / steps
            pts.append((cx + d_outer * math.cos(a), cy - d_outer * math.sin(a)))
        draw.polygon(pts, fill=colors[i % 2])

    # Coloured rings – draw as filled arcs by layering
    def filled_ring(r_out, r_in, color):
        bb_out = [cx - r_out, cy - r_out, cx + r_out, cy + r_out]
        bb_in  = [cx - r_in,  cy - r_in,  cx + r_in,  cy + r_in]
        for i in range(20):
            a0_deg = math.degrees(i * angle_increment + angle_offset)
            a1_deg = a0_deg + 18
            # Pillow arc uses clockwise from 3 o'clock; adjust sign
            draw.pieslice(bb_out, start=-a1_deg, end=-a0_deg, fill=color)
            draw.pieslice(bb_in,  start=-a1_deg, end=-a0_deg, fill=colors[i % 2])

    filled_ring(int(t_outer), int(t_inner), "red")
    filled_ring(int(d_outer), int(d_inner), "green")

    # Segment dividers
    for i in range(20):
        a = i * angle_increment + angle_offset
        draw.line([(cx, cy),
                   (cx + d_outer * math.cos(a), cy - d_outer * math.sin(a))],
                  fill="black", width=2)

    # Bullseye
    draw.ellipse([cx - outer_bull, cy - outer_bull, cx + outer_bull, cy + outer_bull],
                 fill="green")
    draw.ellipse([cx - inner_bull, cy - inner_bull, cx + inner_bull, cy + inner_bull],
                 fill="red")

    # Outer border circle
    draw.ellipse([cx - d_outer, cy - d_outer, cx + d_outer, cy + d_outer],
    ...
    for i in range(20):
        ...
        draw.text((tx, ty), str(segment_order[i]), fill="white", font=fnt, anchor="mm")
    ...
    draw_x(target_pos, "yellow")
    draw_x(result_pos, "orange")

    return img

def board_coords(click_x_pct, click_y_pct):
    """Convert 0–1 percentages from streamlit-image-coordinates to board offsets."""
    px = click_x_pct * BOARD_PX
    py = click_y_pct * BOARD_PX
    cx = cy = BOARD_PX // 2
    return px - cx, cy - py   # (x_diff, y_diff) matching original convention
...
# ── Install streamlit-image-coordinates if needed ────────────────────────────
try:
    from streamlit_image_coordinates import streamlit_image_coordinates
except ImportError:
    st.error("Missing dependency. Add `streamlit-image-coordinates` to your requirements.txt")
    st.stop()
...
board_img = draw_board(
    target_pos=s.last_target_pos,
    result_pos=s.last_result_pos,
)
...
if click is not None:
    raw_x = click["x"]
    raw_y = click["y"]
    # streamlit-image-coordinates returns pixel coords directly
    cx_px = BOARD_PX // 2
    cy_px = BOARD_PX // 2
    x_diff = raw_x - cx_px
    y_diff = cy_px - raw_y
    ...
    if s.recording_target:
        ...
        s.last_target_pos     = (x_diff, y_diff)
        s.last_result_pos     = None
        ...
        s.last_result_pos  = (x_diff, y_diff)
