...
def tab_points(df):
    ...
    include_all_modes = st.toggle(
        ...
        value=False,
        key="points_include_all_modes",
        help="Off = Points only. On = use all currently filtered data.",
    )
...
def build_consecutive_hit_streaks(source_df):
    ...
    working = source_df.reset_index().rename(columns={"index": "Row Order"}).copy()
...
def tab_rtw(df):
    ...
    include_all_modes = st.toggle(
        "Include all modes in this report",
        value=False,
        key="rtw_include_all_modes",
        help="Off = RTW only. On = use all currently filtered data.",
    )
    ...
    if streak_summary.empty:
        ...
        fig4 = px.bar(
            streak_summary.sort_values("Streak Length"),
            x="Count",
            y="Label",
            orientation="h",
            text="Count",
            color="Streak Length",
            color_continuous_scale=["#4f98a3", "#01696f"],
            labels={"Count": "Number of streaks", "Label": "Consecutive hits"},
        )
        ...
        fig4.update_layout(
            ...
            xaxis=dict(gridcolor="rgba(255,255,255,0.05)"),
            yaxis=dict(type="category"),
            height=max(300, 70 * len(streak_summary)),
            margin=dict(t=10),
        )
        st.plotly_chart(fig4, use_container_width=True)
...
def tab_sessions(df):
    ...
    fig.update_layout(
        ...
        yaxis2=dict(title="Throws", overlaying="y", side="right", showgrid=False),
        height=360,
        margin=dict(t=10),
    )
    st.plotly_chart(fig, use_container_width=True)
...
def tab_raw(df):
    ...
    st.download_button(
        ...
        data=csv,
        file_name="dart_tracker_filtered_data.csv",
        mime="text/csv",
    )
